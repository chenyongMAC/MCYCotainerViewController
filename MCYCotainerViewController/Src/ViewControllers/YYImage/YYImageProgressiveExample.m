...
- (void)changed {
    ...
    NSData *data = [NSData dataWithName:name];
    float progressive = _slider0.value;
    if (progressive > 1) {
        progressive = 1;
    }
    NSData *subData = [data subdataWithRange:NSMakeRange(0, data.length*progressive)];
